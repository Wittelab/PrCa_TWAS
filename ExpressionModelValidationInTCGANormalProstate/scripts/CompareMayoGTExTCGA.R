...
for( gene_name in genes_in_common ){
  ...
  if( !exists("gtex_vs_tcga") ){
    gtex_vs_tcga = data.frame(gene=character(), effect=double(), stderr=double(), pval=double(), mse=double())
    gtex_vs_tcga_row = data.frame(gene=gene_name, effect=gtex_vs_tcga_model$coefficients[2,1], stderr=gtex_vs_tcga_model$coefficients[2,2], pval=gtex_vs_tcga_model$coefficients[2,4], mse=ssr)
  ...
  if( !exists("mayo_vs_gtex") ){
    mayo_vs_gtex = data.frame(gene=character(), effect=double(), stderr=double(), pval=double(), mse=double())
    mayo_vs_gtex_row = data.frame(gene=gene_name, effect=mayo_vs_gtex_model$coefficients[2,1], stderr=mayo_vs_gtex_model$coefficients[2,2], pval=mayo_vs_gtex_model$coefficients[2,4], mse=ssr)
