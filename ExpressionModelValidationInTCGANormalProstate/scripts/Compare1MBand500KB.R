...
for( gene_name in genes_in_common ){
  ...
  if( !exists("fivehundredkbp_vs_tcga") ){
    fivehundredkbp_vs_tcga = data.frame(gene=character(), effect=double(), stderr=double(), pval=double(), mse=double())
    fivehundredkbp_vs_tcga_row = data.frame(gene=as.character(gene_name), effect=fivehundredkbp_vs_tcga_model$coefficients[2,1], stderr=fivehundredkbp_vs_tcga_model$coefficients[2,2], pval=fivehundredkbp_vs_tcga_model$coefficients[2,4], mse=ssr)
  ...
  if( !exists("onembp_vs_tcga") ){
    onembp_vs_tcga = data.frame(gene=character(), effect=double(), stderr=double(), pval=double(), mse=double())
    onembp_vs_tcga_row = data.frame(gene=gene_name, effect=onembp_vs_tcga_model$coefficients[2,1], stderr=onembp_vs_tcga_model$coefficients[2,2], pval=onembp_vs_tcga_model$coefficients[2,4], mse=ssr)
  ...
  if( !exists("fivehundredkbp_vs_onembp") ){
    ...
    fivehundredkbp_vs_onembp_row = data.frame(gene=gene_name, effect=fivehundredkbp_vs_onembp_model$coefficients[2,1], stderr=fivehundredkbp_vs_onembp_model$coefficients[2,2], pval=fivehundredkbp_vs_onembp_model$coefficients[2,4], mse=ssr)
