...
for( gene_name in genes_in_common ){
  ...
  if( !exists("tenfold_vs_tcga") ){
    tenfold_vs_tcga = data.frame(gene=character(), effect=double(), stderr=double(), pval=double(), mse=double())
    tenfold_vs_tcga_row = data.frame(gene=as.character(gene_name), effect=tenfold_vs_tcga_model$coefficients[2,1], stderr=tenfold_vs_tcga_model$coefficients[2,2], pval=tenfold_vs_tcga_model$coefficients[2,4], mse=ssr)
  ...
  if( !exists("loocv_vs_tcga") ){
    loocv_vs_tcga = data.frame(gene=character(), effect=double(), stderr=double(), pval=double(), mse=double())
    loocv_vs_tcga_row = data.frame(gene=gene_name, effect=loocv_vs_tcga_model$coefficients[2,1], stderr=loocv_vs_tcga_model$coefficients[2,2], pval=loocv_vs_tcga_model$coefficients[2,4], mse=ssr)
  ...
  if( !exists("tenfold_vs_loocv") ){
    ...
    tenfold_vs_loocv_row = data.frame(gene=gene_name, effect=tenfold_vs_loocv_model$coefficients[2,1], stderr=tenfold_vs_loocv_model$coefficients[2,2], pval=tenfold_vs_loocv_model$coefficients[2,4], mse=ssr)
